...
function fetch() {
  ...
  xhr.open('GET', 'http://bingweb.binghamton.edu/~abhat1/b-line', true);
  //  xhr.open('GET', 'http://api.rottentomatoes.com/api/public/v1.0/lists/movies/upcoming.json?apikey=a42wwqyv364u8hxjecengaqd&page_limit=10', true);

	xhr.onload = function(e) {
		if (xhr.readyState == 4) {
			if (xhr.status == 200) {
				if (xhr.responseText) {
          appMessageQueue.push({'message': {'index': 0, 'content': 'hello'}});
					res = JSON.parse(xhr.responseText);
					res.objects.forEach(function (element, index, array) {
            //ongoing=element.ongoing;
						ongoing = element.ongoing.substring(0,159);
						appMessageQueue.push({'message': {'index': index, 'content': ongoing}});
					});
				} else {
